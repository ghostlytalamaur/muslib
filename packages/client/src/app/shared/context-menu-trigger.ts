 ...
 */
@Directive({
  selector: `[appCtxMatMenuTriggerFor]`,
  host: {
    'aria-haspopup': 'true',
...
export class AppCtxMatMenuDirective implements AfterContentInit, OnDestroy {
  ...
  @Input('openMenuByLClick')
  openMenuByLClick: boolean;
  ...
  toggleMenu(pos: Point): void {
    return this._menuOpen ? this.closeMenu() : this.openMenu(pos);
  }
  ...
  openMenu(pos?: Point): void {
    ...
    if (this.menu.lazyContent) {
      console.log('Attaching lazy content.');
  ...
  private _checkMenu() {
    if (!this.menu) {
      // noinspection AngularInvalidTemplateReferenceVariable
  ...
  private _createOverlay(pos?: Point): OverlayRef {
    if (!this._overlayRef) {
      ...
      // this._overlayRef.updatePositionStrategy(this.getPositionStrategy(pos));
    }
