...
function mockServices(): { artistsService: ArtistsService, albumsService: AlbumsService } {
  const artistsService = mock(ArtistsService);
  const albumsService = mock(AlbumsService);
  for (const dataSet of data) {
    when(artistsService.getArtist(dataSet.artist.id))
      .thenReturn(of(dataSet.artist));

    for (const album of dataSet.albums) {
      when(albumsService.getAlbums(dataSet.artist.id))
        .thenReturn(of(dataSet.albums));
      when(albumsService.deleteAlbum(dataSet.artist.id, album.id))
        .thenResolve();
    }
  }
...
describe('ArtistDetailsComponent', () => {
  ...
  const {artistsService: artistServiceMock, albumsService: albumsServiceMock} = mockServices();
  ...
  beforeEach(async(() => {
    TestBed.configureTestingModule({
      ...
      providers: [
        {provide: ArtistsService, useFactory: () => instance(artistServiceMock)},
        {provide: AlbumsService, useFactory: () => instance(albumsServiceMock)},
  ...
  it('should delete album', () => {
    ...
    verify(albumsServiceMock.deleteAlbum(data[0].artist.id, data[0].albums[0].id)).once();
  });
