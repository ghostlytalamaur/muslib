...
import {MatProgressBar, MatProgressBarModule, MatToolbarModule} from '@angular/material';
...
describe('AppNavBarComponent', () => {
  ...
  beforeEach(() => {
    ...
    linksDe = fixture.debugElement.queryAll(By.directive(RouterLinkStubDirective));
  ...
  it('should have proper links', () => {
    ...
    for (let i = 0; i < expectedLinks.length; i++) {
      expect(routerLinks[i].linkParams).toBe(expectedLinks[i], `Invalid navigation link #${i}`);
    }
  });
  ...
  it('should navigate to Library', () => {
    for (let i = 0; i < expectedLinks.length; i++) {
      ...
      (linksDe[i].nativeElement as HTMLElement).dispatchEvent(new MouseEvent('click'));
      expect(routerLinks[i].navigatedUrl).toBe(expectedLinks[i],
  ...
  it('should show/hide progress bar according to active operations', () => {
    ...
    expect(fixture.debugElement.query(By.directive(MatProgressBar)))
      .toBeFalsy('progress bar should not shows without running operations');
    srv.startOperation();
    try {
